...
struct SoldProductsView: View {
    @State private var soldProducts: [Product] = []
    ...
    private func fetchData() {
        ...
        let soldProductsRef = db.collection("products")
            .whereField("sellerId", isEqualTo: userId)
            .whereField("status", isEqualTo: "unavailable")

        soldProductsRef.addSnapshotListener { querySnapshot, error in
            guard let documents = querySnapshot?.documents else {
                print("Error fetching documents: \(error!)")
                return
            }

            soldProducts = documents.map { document -> Product in
...
struct SoldProductsView_Previews: PreviewProvider {
